...
public class Exo1 {
	...
	public static void execute() {
		...
		Suspect a = new Suspect("Ann", 30, 10);
		G.addVertex(a);
		Suspect b = new Suspect("Betty", 50, 25);
		G.addVertex(b);
		Suspect c = new Suspect("Cynthia", 50, 50);
		G.addVertex(c);
		Suspect d = new Suspect("Diana", 30, 65);
		G.addVertex(d);
		Suspect e = new Suspect("Emily", 10, 50);
		G.addVertex(e);
		Suspect f = new Suspect("Felicia", 10, 25);
		G.addVertex(f);
		Suspect g = new Suspect("Georgia", 80, 10);
		G.addVertex(g);
		Suspect h = new Suspect("Helen", 80, 50);
		G.addVertex(h);
		...
		// On affiche le graph
		displayColoredGraph("Exo 1", G, new Dimension(680, 550));

		// On détermine la coupable
		Suspect max = null;
		for (Suspect s : (Collection<Suspect>) G.getVertices()) {
			if (max == null || max.getSuspicionLevel() < s.getSuspicionLevel())
				max = s;
		}
	...
	public static void displayColoredGraph(String title, UndirectedGraph<Suspect, Rencontre> graph,
			Dimension dimension) {
		// On calcule la valeur max de suspicion
		int i = 0;
		for (Suspect s : (Collection<Suspect>) graph.getVertices())
			i = Math.max(i, s.getSuspicionLevel());
		final double max = i;
		...
		GraphUtils.displayGraph(title, graph, dimension, false, ts, false);
	}
			...
			for (Suspect s2 : n1) {
				...
				for (Suspect s3 : g.getNeighbors(s2)) {
					...
					if (ss.contains(s3)) {
						...
						// On ne fait plus ce test : le fait de conserver les
						// redondances permet
						// d'augmenter la suspicion
						// if (rs.contains(r)) continue;
						rs.add(r);
						System.out.println(String.format(
								" - %s et %s ne se sont pas rencontreés, alors que %s les a vu tt les deux", s1, s3,
								s2));
						s1.addSuspicion();
						s3.addSuspicion();
					}
